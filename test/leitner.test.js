...
test("asks from the lowest non-empty box, whatever the other boxes hold", () => {
  assert.equal(pickBox([90, 2, 1, 0, 0, 0], 1, fixed(0.99)), 0);
  assert.equal(pickBox([0, 5, 3, 0, 0, 80], 7, fixed(0.99)), 1);
});
...
test("every 10th question reviews a higher box, weighted count/2^i", () => {
  ...
  assert.equal(pickBox(counts, 10, fixed(0.5)), 1);
  assert.equal(pickBox(counts, 20, fixed(0.9)), 2);
});

test("a review with no higher box asks from the only box", () => {
  assert.equal(pickBox([0, 0, 3, 0, 0, 0], 10, fixed(0.99)), 2);
});
